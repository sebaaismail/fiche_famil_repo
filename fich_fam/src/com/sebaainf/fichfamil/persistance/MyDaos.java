...
public class MyDaos {
    ...
    static String database = "dbfichefamil";
     ...
     * @param id_mar
     * @return
     * @should return enfants de mariage avec id_mar
     */
    public static TreeSet<Enfant> getEnfants(int id_mar) {

        List<Enfant> listEnfs;
        TreeSet<Enfant> enfants = null;

        try {
            String sql = "select * from enfant where id_mar=?";
            IDaos daos = MyDaos.persistenceManager.createDaos();
            listEnfs = daos.getObjectDao().readList(Enfant.class, sql, id_mar);

            enfants = new TreeSet<Enfant>(listEnfs);
